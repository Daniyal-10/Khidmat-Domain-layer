...
4. **Tier 1 sources themselves** (`KHIDMAT_BUSINESS_LOGIC_BLUEPRINT_V1.md`, `KHIDMAT_AI_BUSINESS_OVERVIEW.html`) — untouched. G1 amends the *ontology's* treatment of BL V1 §4, per the README's own amend-at-source rule (XCR-2), by recording the correction in `04-ARCHITECTURE-RULES.md` (a §1-style synchronization entry) rather than editing the frozen Blueprint text itself.
