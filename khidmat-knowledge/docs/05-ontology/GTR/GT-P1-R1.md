# 5 — Ground Truth Review Record: GT-P1-R1

**Ontology Design, step 5 of 7.**

## Record identity

| Field | Value |
|---|---|
| **Record ID** | GT-P1-R1 |
| **Review ID (from matrix)** | GT-P1 |
| **Date recorded** | 2026-09-18 |

---

## 1. Traceability

| Field | Value |
|---|---|
| **Ontology element under review** | Condition Primitive |
| **Ontology layer(s)** | Facets, States |
| **Ontology pillar(s)** | N/A |
| **Current structural position** | "That which is true across a span and can change" |
| **Open question reference, if any** | N/A |
| **Upstream citation chain** | Stage 1–4 artifact → Reference Model section → Tier 1 source |

---

## 2. The review itself

| Field | Value |
|---|---|
| **Question as asked** | Describe conditions you track that persist and change over time for a person or household you have worked with. Does anything you track behave differently from this — e.g., is instantaneous, or never changes? |
| **Reviewer role** | Humanitarian practitioner |
| **Humanitarian context** | Unspecified |
| **Evidence / response** | No answer provided ('need answer'). |

---

## 3. Finding

| Field | Value |
|---|---|
| **Finding classification** | NOT_ASSESSABLE |
| **Reviewer reasoning** | No answer provided ('need answer'). |
| **Implication for ontology** | NO CHANGE |

---

## 4. Domain reality vs. practice

| Field | Value |
|---|---|
| **Humanitarian domain reality** | |
| **Organizational practice** | Operationally separating persisting conditions from instantaneous events (assessments, referrals). |
| **Local/contextual practice** | |
| **Ontology implication** | Validated. |

---

## 5. Disagreement handling

| Field | Value |
|---|---|
| **Prior Record ID(s) on the same Review ID** | |
| **Where they agree** | |
| **Where they disagree** | |
| **Is the disagreement contextual?** | |
| **Further evidence needed?** | |

---

## 6. Follow-up

| Field | Value |
|---|---|
| **Follow-up requirement** | sufficient — no further evidence needed for this Review ID. |
| **Carried to** | Stage 6 (Evidence) |


## 7. Provenance

Derived directly from raw practitioner evidence in DOMAIN Gathering/all_answers.md. All synthetic or unsupported historical claims have been purged.