...
- **Secondary:** AI collaborators operating under this project's multi-agent collaboration workflow — they need this document as the checkable source of truth for "is this concept business-real or an invented convenience?"
...
- Software architecture of any kind (this repository's own future workflow and architecture documentation will govern that separately, for the knowledge layer specifically — the Business Master Plan is upstream of even that).
- Anything that names a YAML file, a file path, or a repository directory as part of its own content. If a sentence in the Business Master Plan could not be understood by someone who has never seen this repository, it is in scope. If it requires having read a future ontology-engineering schema document to parse, it is out of scope.
...
2. The multi-agent AI collaboration roles this project's workflow defines.
...
This chain supplements, but does not amend, `CONSTITUTION.md` Article XVI, which remains the sole authoritative dependency hierarchy; Vision's placement here is descriptive context, not a constitutional gate.
...
- **Why it exists:** Ontology engineering will eventually have to resolve tensions like "does a safety flag trigger assessment or automatic closure" (a speed-versus-safety tension). The BMP should name the tension *before* the ontology resolves it, so future tensions are recognized and deliberated rather than resolved ad hoc during implementation.
...
- **Why it exists:** Ontology engineering will eventually need a *technical* lifecycle model (a stage graph, an engagement-stage taxonomy). Neither exists yet, and neither would be a business description on its own — a business-level lifecycle written first would let a future reviewer check whether that eventual technical model is a faithful rendering of it.
...
- **Expected output:** A stage-by-stage business narrative (not a state machine) of how a humanitarian case unfolds, written independently of any future technical engagement-stage taxonomy so that taxonomy can later be checked against it.
...
- **Why it exists:** Capabilities alone describe *ability*; value streams describe *flow*. Any future ontology's cross-domain relationships will be technical renderings of value-stream connections that should first be describable in a sentence a non-technical stakeholder could follow.
...
- **Why it exists:** Any future support-intervention taxonomy will need this business grounding. The intervention catalogue is expected to require operational input from programme staff before it can be finalized — this chapter should capture the business-level knowledge that input depends on.
...
- **Why it exists:** This is the direct business-level ancestor of the still-unwritten outcome-indicator vocabulary, which will remain blocked until this kind of cross-domain business agreement exists.
...
- **Why it exists:** Ontology engineering will eventually have to make this call, concretely — whether regional/localized labels are aliases or new concepts is a decision that needs a preceding business-level statement of *which humanitarian variations are surface-level labeling and which are substantively different practices*. This chapter provides that statement in advance rather than after the fact.
- **Dependencies:** All prior chapters (this is a cross-cutting check on everything already described).
- **Expected output:** An explicit universal-vs-variable distinction, chapter by chapter if needed, that any future regional-localization decision and future regional extension work (e.g., an eventual Islamic-giving model) can be checked against.
...
- Once a business-level lifecycle is written, and a technical engagement-stage taxonomy is later authored from it, does that taxonomy genuinely match the business-level stages, or does it introduce implementation-driven simplifications a business-first description would not have produced independently?

**For Chapter 4 (Capabilities):**
- Is a future domain list a complete rendering of humanitarian business capabilities, or might some capabilities go unmodelled because no operational trigger has occurred yet?
...
- What distinguishes a "time-limited" from a "sustained" intervention in business terms, independent of however a future household-resilience ontology might infer it?
...
**Additional discovery topics (merged from an archived, invalid review cycle — see `docs/98-archive/`; these four are retained because they are independently grounded in `docs/98-archive/execution_cleanup/BUSINESS_ARCHITECTURE_BLUEPRINT.md`, a document that genuinely exists, not in the invalid review itself):**
...
These are architectural/process questions this blueprint surfaces but cannot resolve on its own — they require a decision from the Human Owner before or during BMP authoring:
...
2. **Retroactive vs. forward-looking scope:** Should the BMP describe humanitarian business reality purely as a timeless domain description (as this blueprint has assumed throughout), or should it also explicitly account for any ontology-level business logic decided later, so nothing already-ratified becomes silently contradicted?
...
4. **Relationship to `KHIDMAT_BUSINESS_LOGIC_BLUEPRINT_V1.md`:** The repository (at `docs/98-archive/execution_cleanup/BUSINESS_ARCHITECTURE_BLUEPRINT.md`) already contains a populated document with "business logic" in its name. Before the new BMP is authored, is this document intended to be superseded by it, to remain as a separate (perhaps more product-oriented) document, or to be a direct input? This should be settled explicitly rather than discovered mid-authoring.
...
- **Treat any future architectural decision record as a business-reality source, not just an engineering record.** Wherever a chapter's topic overlaps with a decision made there, the chapter should be checked against it for consistency — not to import ontology language, but to make sure the business description doesn't accidentally contradict a decision the client has already ratified for a good business reason.
- **Flag, don't guess, on every Knowledge Required item (§7).** Where a discovery topic in §7 has not actually been resolved by the time a chapter is drafted, the chapter should say so explicitly (a stated open assumption) rather than silently inventing an answer.
...
4. Every open question in §8 has been explicitly answered by the Human Owner (or delegated decision-maker) and that answer is recorded (as an architectural decision record, if it is an architectural decision).
5. A cross-check has been performed against every architectural decision made by the time of authoring whose subject matter overlaps a BMP chapter, confirming no contradiction — documented as a short traceability note per chapter, not a rewrite of those decisions.
