...
```
KHIDMAT_AI_PROJECT_OVERVIEW.md  (why the project exists — informal, working draft)
        ↓
...
|---|---|---|
| `KHIDMAT_AI_PROJECT_OVERVIEW.md` | Informal, evolving statement of the project's current understanding — a working alignment point, explicitly *not* any formal document. | The Overview is provisional and personal-voice; the BMP is the first *normative*, stable business description. The Overview may eventually be retired once the formal chapters (VISION, PHILOSOPHY, BMP, etc.) collectively supersede it — that retirement decision is out of scope here. |
...
4. **Relationship to `KHIDMAT_BUSINESS_LOGIC_BLUEPRINT_V1.md`:** The repository root already contains a populated document with "business logic" in its name. Before the new BMP is authored, is `KHIDMAT_BUSINESS_LOGIC_BLUEPRINT_V1.md` intended to be superseded by it, to remain as a separate (perhaps more product-oriented) document, or to be a direct input? This should be settled explicitly rather than discovered mid-authoring.
