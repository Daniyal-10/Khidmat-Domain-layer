...
| [025](ADR-025-donor-identity-model.md) | Donor Identity Model | Accepted | Donor is modeled as a role/profile (`donor_profile`) attached behind `shared:person` or `shared:organisation` — never an independent entity — mirroring the `volunteer_profile`/`shared:actor` pattern. Applied to the Donor & Resource domain (HKMP Stage 8). |
| [026](ADR-026-volunteer-boundary.md) | Volunteer Boundary (Donor & Resource / Volunteer Operations) | Accepted | Donor & Resource authors zero volunteer-related content; every overlapping concept named in the Stage 8 brief is already owned by Volunteer Operations' foundational tier or deferred to its Tier 2. |
| [027](ADR-027-grant-ownership.md) | Grant Ownership | Accepted | `grant` is an optional abstraction between Programs and Organisation/Donor Profile, coexisting with — not replacing — the existing `program_funded_by` edge. Closes Programs' own disclosed Grant Management gap without modifying certified Programs content. |
| [028](ADR-028-resource-model.md) | Resource Model | Accepted | A shallow `resource` (abstract) → `financial_resource`/`material_resource` hierarchy, mirroring `subject`/ADR-018, with `inventory_item` as the non-negotiable type/instance split. Applied to the Donor & Resource domain's Material Resource & Logistics sub-stage (HKMP Stage 8C). |
| [029](ADR-029-case-registration-record-reconciliation.md) | Case / Registration Record Reconciliation — One Canonical Case Stands | Proposed (pending Owner ratification) | Reconciles ADR-021 vs. the Phase 1.1A split by upholding ADR-021: one canonical `Case` owned by Case Management, no separate `Registration Record` entity. Formalized from the retired `ADR_RECONCILIATION_CASE.md`. |
