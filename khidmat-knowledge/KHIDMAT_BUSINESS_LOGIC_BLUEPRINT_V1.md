...
> **Amendment (2026-09-02), per Stage 7 Governance ruling G1.** The original text above collapsed Organisation and Programme into a single actor row. The Organisation/Programme split is retained as standard structural modeling—an Organisation (the operating body) and a Programme (a specific initiative it may run) are naturally distinct concepts. Khidmat's own current operation is scoped as a single Organisation with no Programmes. This amendment separates the structural design from Khidmat's specific operational scope and is made at source per this document's own `XCR-2` amend-at-source discipline (see `docs/05-ontology/04-ARCHITECTURE-RULES.md` §4.6).
