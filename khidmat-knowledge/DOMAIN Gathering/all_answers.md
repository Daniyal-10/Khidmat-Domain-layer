From the survey we get the lead that can be told to us in any way that they are in really need 
Name number address and what really needed (Food, medical , shelter , education )

Then the khidmat grp volunteerss (the staff)
Visits the beneficiary to do the ground reality check 
Asper the need and income source members their IDs are taken form is filled of the verfication and identification 
...
documents: 1.aadhar, 2.voter ID, 3. Ayushman, 4. Permit (checkbox)
...
THIS IS WORKING IF first come first serve 

also from the donor side asper the needed decided doantions the donors can combine or split to help the beneficaries 

Also 2 3 vendors are there connected with them so the beneficiary can visit them and take the ration (from the card) 

Donors can make the donation via the cards
Also if not present at the moment (the another person can be send by the donor)

Yes donors have limit to donate is some cases so that the needy once don't become too much dependent on other 
...
Mostly families are covered as we are taking about the help of the family based and verified persons 
Household condition can be considered (for the family only) but it is rare and its the volunteer take 
...
currently the full flow of their khidmat is 
first the survey is done (I have asked for more details in this )
then after this the one who are listed for the verification then those beneficary are visited by the volunteers to get all the details and fill the khidmat form so here the gorund verification is done 
as the form is filled and ground check is also done by the volunteer the form goes is uploaded in a grp where the donors are there and then here the first come first server is who says that he will help them they are taken as resposible for that particaulr beneficary 

also about the reverifcation mostly the families and the needy once that we help are too humble families soo there earning members maybe take more longer and even if they have still some time we help them with the bare minimum need to food
...
it is a very simple flow survey is there are 2 grps one where the survey (leads) are there and the khidmat grp staff members are there (volunteers) where based on that they do the verifaction on the ground and then after the lead is verified then it is uploaded in the 2nd grp where the donors are there and the khidmat memebers 
there the donors adopt the beneifcary (based on the above detialed things ) and then simply the requiremets are fullfilled 
